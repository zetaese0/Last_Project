...
const Login = () => {
  ...
  useEffect(() => {
    console.log(store.tokenAdmin);
    store.tokenAdmin && navigate("/admin/private");
    return () => {
      setUser({
        name: "",
        email: "",
  ...
  const registerAdmin = async () => {
    if (user.password == user.password_check && user.password != "") {
      const createUser = await actions.createAdmin(user);
      if (createUser) {
  ...
  const loginAdmin = async () => {
    const login = await actions.loginAdmin(user);
    console.log(login);
    if (login) {
      ...
      <section onSubmit={(e) => e.preventDefault}>
        ...
        <div className="action">
          <button
            ...
            onClick={() => (isShow ? registerAdmin() : setIsShown(!isShow))}
          >
            Register
          </button>
          <button
            className={isShow ? "" : "selected"}
            onClick={() => (!isShow ? loginAdmin() : setIsShown(!isShow))}
          >
            Sign in
          </button>
        </div>
